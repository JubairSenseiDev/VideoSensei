...
### 🛠️ Installer overhaul

- **New `--yes` flag** for non-interactive installs (CI / unattended / `curl|bash` pipes). Auto-confirms any prompt.
- **Local installer fallback** in `install.sh` — uses the bundled `installer/installer.sh` when present, skipping the network round-trip.
- **TTY-aware prompt** — `install.sh` now only redirects stdin from `/dev/tty` when running interactively. In CI (no TTY), it auto-passes `--yes`.
- **Cleaner `--help`** output — adds `--yes` and `--uninstall` to usage.
- **Bumped installer version to 1.3.1.**

### 🎨 Banner

- Replaced hand-crafted ASCII banner (had letter-rendering mistakes) with one generated by `pyfiglet` using the `doom` font.
- Whole word `VIDEOSENSEI` now fits on a single 67-column line — no more wrap to two lines.

### 🌿 Repository structure

- Split into three clear branches:
  - `main` — what users install (CLI + installer + Termux guide)
  - `dev` — contributor docs (architecture, branding, theme, roadmap, ui-mockups, compression)
  - `flutter-app` — Flutter GUI source + multi-platform CI (WIP)
- Main README simplified to install + usage + help links only.

### 🐛 Fixed

- `install.sh` previously failed under `curl | bash` because `/dev/tty` redirect hung in non-interactive shells.
- Hardcoded `1.2.0` version string in `package.json` `build:bun:*` scripts — now `1.3.0`.
- Installer `test-installer` job in CI was failing → release job was skipped → **no GitHub release was ever created**. Now fixed by `--yes` flag.
